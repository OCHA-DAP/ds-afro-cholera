...
## Methodology 4
Create a 3-year rolling average of weekly caseload with a minimum of 500 new cases. 
...
#  dplyr$group_modify(~ {
#    df <- .x
#    # Use absolute cases as the signal
#    signal <- df$cholera_cases
#    # Keep only non-negative values; others -> NA
#    valid_cases <- dplyr$if_else(signal >= 0, signal, NA_real_)
#    n_valid <- sum(!is.na(valid_cases))
#    # 3-year (~156 weeks) rolling mean of absolute cases
#    if (n_valid >= 156) {
#      roll <- zoo$rollmean(valid_cases, k = 156, fill = NA, align = "right")
#      # forward-fill initial NAs with first non-NA rolling value
#      first_val <- roll[which(!is.na(roll))[1]]
#      roll[is.na(roll)] <- first_val
#    } else {
#      # fallback: constant baseline = mean of available cases
#      roll <- rep(mean(valid_cases, na.rm = TRUE), length(valid_cases))
#    }
#    df$roll_3yr <- roll
#    # Alert when current cases exceed rolling baseline and a minimum absolute threshold
#    df$alert <- df$cholera_cases >= df$roll_3yr & df$cholera_cases >= min_cases
#    df$year <- lubridate$year(df$date)
#    df
#  }) |>
#  dplyr$ungroup()
...
ggplot2$ggplot(df_4_aggregated, ggplot2$aes(x = year, y = iso3, fill = num_alerts)) +
  ...
  ggplot2$scale_fill_gradient(low = "white", high = "red", limits = c(0, 15),
                              breaks = function(x) x[x %% 1 == 0],
                              labels = function(x) x[x %% 1 == 0]) +
...
Challenges with the Rolling Average Method (3-year baseline)

 1. Data Gaps and Inconsistencies
  - Many countries lack continuous weekly data over 3 years.
  - Missing weeks break the continuity, making the rolling mean less reliable.
 2. Short Time Series
  - For countries with only 1–2 years of data, the method results in a simple mean value.
 3. Small spikes in cases can exceed the rolling mean, generating alerts even when there is no meaningful outbreak.
 4. Multiple Alerts for the Same Event
 
#### Timeline using the weekly increase in cases
  ...
  print({
    ggplot2$ggplot(df_country, ggplot2$aes(x = date, y = weekly_increase)) +
      ...
      ggplot2$labs(
        ...
        ● Green curve: 3-year rolling average OR average of available years"
      ) +
      gghdx$gghdx() +
      ggplot2$theme(
...
#### Timeline using the cumulative cases

```{r rolling-average-tl-cases, echo=FALSE, warning=FALSE}
df_4 <- df_prepped |>
  dplyr$group_by(iso3) |>
  dplyr$group_modify(~ {
    df <- .x
...
purrr$walk(iso3_to_plot, ~ {
  ...
  df_country <- df_4[df_4$iso3 == .x, ]
  print({
    ggplot2$ggplot(df_country, ggplot2$aes(x = date, y = cholera_cases)) +
      ggplot2$geom_line(size = 0.6, color = "grey40") +
      ggplot2$geom_line(ggplot2$aes(y = roll_3yr), color = "green4", size = 0.5, na.rm = TRUE) +
      ggplot2$geom_point(data = subset(df_country, alert == TRUE),
