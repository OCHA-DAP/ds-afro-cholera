...
df_cerf <- readr$read_csv(file.path(Sys.getenv("AA_DATA_DIR"),
                                    ...
                                    "250509_CERF cholera allocations(Data).csv"))
...
df_1_all <- df_prepped |>
  ...
  dplyr$mutate(
    ...
    alert_level = dplyr$case_when(
      weekly_increase >= 500 & weekly_increase >= p99 & cholera_cfr > 1 ~ "p99",
      weekly_increase >= 500 & weekly_increase >= p975 ~ "p975",
      weekly_increase >= 500 & weekly_increase >= p95 ~ "p95",
      TRUE ~ "none"
    )
  )
df_1 <- df_1_all |>
  ...
  dplyr$mutate(
    ...
    alert_initial = purrr$map_lgl(date, ~{
      ...
      if (all(is.na(window_vals))) {
        ...
        (current_val == max_val) && (current_val >= 500) && (cholera_cfr > 1)
      }
    }),
    alert = purrr$map2_lgl(date, seq_along(date), ~{
      if (!alert_initial[.y] || weekly_increase[.y] < 500) return(FALSE)
...
# final proposal
# watch alert: highest value in the last 12 months with min of 100 cases
...
ggplot2$ggplot(heat_df, ggplot2$aes(x = year, y = iso3, fill = num_alerts)) +
  ...
  ggplot2$labs(
    title = "Number of Alerts per Country and Year (min CFR 1%)",
    x = "Year",
    y = "Country",
    fill = "Number of Alerts",
...
iso3_to_plot <- c("CMR", "COD", "ETH", "MOZ", "NGA") #iso3_levels
