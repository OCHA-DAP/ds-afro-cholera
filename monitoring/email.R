send_email <- function(){
  ...
  if (length(watch_blocks) > 0) {
    section_parts <- c(section_parts, "<h3><strong>Watch Alerts</strong></h3> <p>Watch Alerts signal the highest weekly reported new cases in the past 12 months, with at least 500 new cases.</p>", paste(watch_blocks, collapse = "\n"))
  }
  if (length(warning_blocks) > 0) {
    section_parts <- c(section_parts, "<h3><strong>Warning Alerts</strong></h3> <p>Warning Alerts indicate an exceptionally high weekly reported new cases, surpassing the historical 99th percentile and exceeding 500 new cases.</p>", paste(warning_blocks, collapse = "\n"))
  }
